...
if($link === false){
    ...
    $sql = "SELECT grno, bbname, bbmname, mobilenumber, address FROM bloodbank";
...
if (mysqli_num_rows($result) > 0) {
	echo "<h2>Update Blood Bank List</h2>";
    // output data of each row
    echo "<table border=2>";
    echo "<tr><th>Name</th><th>Manager Name</th><th>Address</th><th>Mobile Number</th><th>Action</th></tr>";
    while($row = mysqli_fetch_assoc($result)) {
	echo "<tr>";
        echo "<td>". $row["bbname"]. "</td><td>" . $row["bbmname"]. "</td><td>" . $row["address"]. "</td><td>" . $row["mobilenumber"]. "</td><td>"."<a href='update_blood_bank_list_process.php?id=".$row["grno"]."'><button>Edit</button></a>"."   "."<a href='delete_blood_bank_list.php?id=".$row["grno"]."'><button>Delete</button></a>"."</td></tr>";
	echo "</tr>";
    }
echo "</table>";
} else {
